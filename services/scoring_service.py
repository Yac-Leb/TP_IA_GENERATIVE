...
if __name__ == "__main__":
    ...
    # Exemple de caractéristiques d'un élément
    caracteristiques_element = {
        'energy': 85,
        'danceability': 75,
        'valence': 70
    }
    ...
    # Test du score audio
    score_audio = scoring.calculer_score_audio_match(preferences, caracteristiques_element)
    print(f"\nScore audio match: {score_audio:.3f}")
    
    # Test du score mood
    score_mood = scoring.calculer_score_mood_match(
        "Je veux de la musique énergique",
        "énergique",
        ["énergique", "motivant"]
    )
    print(f"Score mood match: {score_mood:.3f}")
    
    # Test du score global
    score_global = scoring.calculer_score_global(
        similarite_semantique=0.85,
        score_mood=0.90,
        score_preferences=0.92,
        score_audio=0.88
    )
    print(f"Score global: {score_global:.3f}")
    
    print("\n✓ Poids utilisés:")
    for key, val in scoring.weights.items():
        print(f"  - {key}: {val}")
    
    print("\n✓ Test du système de scoring réussi!")
