...
if __name__ == "__main__":
    ...
    # Créer le service (sans clé API pour test)
    service = GeminiService()
    
    if service.model is not None:
        # Test enrichissement texte court
        texte_court = "musique calme"
        print(f"\nTest enrichissement:")
        print(f"  Texte original: '{texte_court}'")
        
        try:
            texte_enrichi = service.enrichir_texte_court(texte_court)
            print(f"  Texte enrichi: '{texte_enrichi}'")
        except Exception as e:
            print(f"  ⚠ Non testé: {e}")
        
        # Test avec texte long (ne devrait pas enrichir)
        texte_long = "Je cherche de la musique calme et relaxante pour travailler"
        print(f"\nTest avec texte long:")
        print(f"  Texte: '{texte_long}'")
        result = service.enrichir_texte_court(texte_long)
        print(f"  Résultat: {'Pas enrichi (assez long)' if result == texte_long else 'Enrichi'}")
    else:
        print("\n⚠ Service Gemini non disponible pour les tests")
        print("  Configurez GEMINI_API_KEY dans .env pour tester")
    
    print("\n✓ Structure du service Gemini créée!")
