...
class QuestionnaireService:
    ...
    def afficher_questionnaire(self):
        """Affiche le questionnaire complet dans la console"""
        pass
    
    def collecter_reponses_console(self) -> Dict[str, Any]:
        """Collecte les réponses via la console (mode interactif)"""
        print("\n" + "="*70)
        print("  RÉPONDEZ AU QUESTIONNAIRE")
        print("="*70)
        ...
        # Questions Likert
        print("\nPARTIE 1 : PRÉFÉRENCES SUR ÉCHELLE (1-5)")
        print("-" * 70)
        ...
        for q in self.questions_likert:
            while True:
                ...
                print(f"{q['echelle']}")
                try:
                    reponse = int(input("Votre réponse (1-5) : "))
                    if 1 <= reponse <= 5:
                        reponses["likert"][q["id"]] = {
                            "valeur": reponse,
        ...
        # Questions ouvertes
        print("\n" + "="*70)
        print("PARTIE 2 : QUESTIONS OUVERTES")
        print("-" * 70)
        
        for q in self.questions_ouvertes:
            print(f"\n{q['question']}")
            print(f"({q['placeholder']})")
            reponse = input("Votre réponse : ").strip()
            
            if q["type"] == "liste":
                # Nettoyer et séparer les éléments
                reponse = [item.strip() for item in reponse.split(",") if item.strip()]
            
            reponses["ouvertes"][q["id"]] = {
                "valeur": reponse,
                "type": q["type"]
            }
    ...
    def sauvegarder_reponses(self, reponses: Dict[str, Any], user_id: str = None) -> Path:
        ...
        print(f"✓ Réponses sauvegardées dans {filepath}")
        return filepath
...
if __name__ == "__main__":
    ...
    service.afficher_questionnaire()
    
    print("\n" + "="*70)
    print("Mode test : utilisation de réponses exemple")
    print("="*70)
    ...
    reponses_exemple = {
        ...
        "ouvertes": {
            "qo1_mood": "Je cherche de la musique énergique et motivante pour mes sessions de sport",
            "qo2_contexte": "Pendant l'entraînement en salle de sport",
            "qo3_artistes": ["Foo Fighters", "The Killers", "Muse"],
            "qo4_genres": ["rock", "alternative", "électro"],
            "qo5_emotions": "Énergie, motivation, puissance"
        }
    }
    
    reponses = service.collecter_reponses_dict(reponses_exemple)
    service.sauvegarder_reponses(reponses, "test_user")
    
    print("\n✓ Préférences audio extraites:")
    prefs = service.extraire_preferences_audio(reponses)
    for key, val in prefs.items():
        print(f"  - {key}: {val}")
    
    print("\n✓ Texte sémantique extrait:")
    texte = service.extraire_texte_semantique(reponses)
    print(f"  {texte}")
