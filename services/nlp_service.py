...
class MoteurNLP:
    ...
    def preparer_referentiel(self, textes_referentiel: List[Dict[str, Any]]):
        ...
        """
        print(f"Préparation du référentiel ({len(textes_referentiel)} éléments)...")
        ...
        print("Encoding des textes du référentiel avec SBERT...")
        self.referentiel_embeddings = self.encoder_textes(textes_bruts)
        
        print(f"✓ Référentiel encodé: {self.referentiel_embeddings.shape}")
    ...
    def _sauvegarder_cache(self):
        ...
        print(f"✓ Cache des embeddings sauvegardé: {self.embeddings_cache_path}")
    ...
    def charger_cache(self) -> bool:
        ...
        try:
            ...
            print(f"✓ Cache des embeddings chargé: {self.referentiel_embeddings.shape}")
            return True
...
if __name__ == "__main__":
    # Test du moteur NLP
    print("Test du Moteur NLP Sémantique")
    print("="*70)
    
    moteur = MoteurNLP()
    
    # Textes d'exemple pour test
    textes_test = [
        {
            "type": "mood",
            "id": "joyeux",
            "text": "joyeux Musique joyeuse et positive pour remonter le moral",
            "data": {}
        },
        {
            "type": "mood",
            "id": "triste",
            "text": "triste Musique mélancolique pour accompagner les moments de tristesse",
            "data": {}
        },
        {
            "type": "ambiance",
            "id": "sport",
            "text": "sport Musique énergique et motivante pour s'entraîner",
            "data": {}
        }
    ]
    
    moteur.preparer_referentiel(textes_test)
    
    # Test de recherche
    requete = "Je veux de la musique pour me motiver pendant mon entraînement"
    print(f"\nRequête: {requete}")
    print("-"*70)
    
    resultats = moteur.rechercher_similaires(requete, top_k=3)
    
    print("\nRésultats:")
    for element, score in resultats:
        print(f"  [{element['type']}] {element['id']}: {score:.3f}")
        print(f"    → {element['text']}")
    
    print("\n✓ Test du moteur NLP réussi!")
